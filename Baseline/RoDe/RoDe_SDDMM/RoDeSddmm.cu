...
void RoDeSDDMM_n256(int m1,int m2,int n,int k,const int* __restrict__ row_indices_block,const int* __restrict__ row_indices_residue,const int* __restrict__ st_offsets,const int* __restrict__ row_offsets,const int* __restrict__ column_indices,\
    const float* __restrict__ values,const float* __restrict__ lhs_matrix,const float* __restrict__ rhs_matrix,float* __restrict__ out,cudaStream_t stream1,cudaStream_t stream2)  {

    RoDeSDDMMKernel_n128<float4,4,32,32,8, 256,32>(m1,m2,n,k,row_indices_block,row_indices_residue,st_offsets,row_offsets,column_indices,values,lhs_matrix,rhs_matrix,out,stream1,stream2);
}