...
class MGCN_dataset(torch.nn.Module):
    ...
    def init_edges(self):
        ...
        self.row_pointers, self.column_index, self.degrees=MagicsphereBlock_cmake.blockProcess8_16_tf32(self.row_pointers1, self.column_index1)
        result = self.row_pointers[2::2] - self.row_pointers[:-2:2]
        self.max=max(result)
        self.degrees= self.degrees.float()
        print()
