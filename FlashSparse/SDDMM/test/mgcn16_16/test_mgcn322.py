...
from mgcn16_16.mdataset_fp32 import *
...
def kernel(inputInfo, epoches, nOri, mOri):
     for i in range(epoches):
        X_prime, spmm_ms_avg =  MagicsphereGAT_kernel.forward_gen_16(inputInfo.x.size(1), inputInfo.num_nodes, inputInfo.row_pointers, inputInfo.column_index, inputInfo.degrees, 
                               inputInfo.x, inputInfo.max, 100)
        print(round(spmm_ms_avg.item(),4))
        res = inputInfo.degrees * 128
        # torch.sum(inputInfo.x[0]*inputInfo.x[0]) == X_prime[0]
