...
def dglGCN(data, data_path, epoches, num_layers, featuredim, hidden, classes):
    sddmm = test_dgl.test(data_path, epoches, num_layers, featuredim, hidden, classes)
    print(str(num_layers) + '-' + str(hidden) + '-' + data + '-dgl-' + '-' + str(sddmm))
    return sddmm
    
#Tcgnn
def tcgnn(data, data_path, epoches, num_layers, featuredim, hidden, classes):
    sddmm = test_tcgnn.test(data_path, epoches, num_layers, featuredim, hidden, classes)
    print(str(num_layers) + '-' + str(hidden) + '-' + data + '-tcgnn-' + '-' + str(sddmm))
    return sddmm
    
#MGPYG
def pygGCN(data, data_path, epoches, num_layers, featuredim, hidden, classes):
    sddmm = test_pyg.test(data_path, epoches, num_layers, featuredim, hidden, classes)
    print(str(num_layers) + '-' + str(hidden) + '-' + data + '-pyg-' + '-' + str(sddmm))
    return sddmm
...
if __name__ == "__main__":
    ...
    for hidden in hidden_list:
        ...
        #result path
        file_name = project_dir + '/result/Baseline/agnn/baseline_agnn_' + str(hidden) + '.csv'
